...
async def promote(event):
    ...
    eventkay = await edit_or_reply(event, "`Promosikan Pengguna Sebagai Admin... Mohon Menunggu`")
    ...
    try:
        ...
        await edit_delete(eventkay, "`Berhasil Menambahkan Pengguna Ini Sebagai Admin!`")

    # If Telethon spit BadRequestError, assume
    # we don't have Promote permission
    except BadRequestError:
        return await eventkay.edit(NO_PERM)
...
async def demote(event):
    ...
    eventkay = await edit_or_reply(event, "`Sedang Melepas Admin...`")
    ...
    # Assume we don't have permission to demote
        ...
        return await eventkay.edit(NO_PERM)
    await edit_delete(eventkay, "`Berhasil Melepas Pengguna Ini Sebagai Admin!`")
...
async def ban(bon):
    ...
    kay = await edit_or_reply(bon, "`Melakukan Banned!`")
...
async def nothanos(unbon):
    ...
    kay = await edit_or_reply(unbon, "`Sedang Melakukan Unban...`")
    ...
    try:
        ...
        await edit_delete(kay, "```Berhasil Melepas Ban Pengguna!```")
        ...
        await edit_delete(kay, "`Sepertinya Terjadi Kesalahan!`")


@kay_cmd(pattern="mute(?: |$)(.*)")
...
async def spider(spdr):
    ...
    kay = await edit_or_reply(spdr, "`Sedang melakukan Mute...`")
    ...
    if user.id == self_user.id:
        return await edit_or_reply(kay,
                                   "`Tangan Terlalu Pendek, Tidak Bisa Membisukan Diri Sendiri...\n(ヘ･_･)ヘ┳━┳`"
                                   )

    # If everything goes well, do announcing and mute
    await kay.edit("`Telah Dibisukan!`")
    if mute(spdr.chat_id, user.id) is False:
        return await kay.edit("`Pengguna Sudah Dibisukan!`")
    else:
        try:
            ...
            if reason:
                await kay.edit(f"**Pengguna Telah Dibisukan!**\n**Alasan:** `{reason}`")
            else:
                await kay.edit("`Telah Dibisukan!`")
            ...
            return await edit_delete(kay, "`Terjadi Kesalahan!`")


@kay_cmd(pattern="unmute(?: |$)(.*)")
...
async def unmoot(unmot):
    ...
    kay = await edit_or_reply(unmot, "```Melakukan Unmute...```")
    ...
    if unmute(unmot.chat_id, user.id) is False:
        ...
        try:
            ...
            await edit_delete(kay, "```Berhasil Melakukan Unmute! Pengguna Sudah Tidak Dibisukan```")
        except UserIdInvalidError:
            return await edit_delete(kay, "`Terjadi Kesalahan!`")
...
async def ungmoot(un_gmute):
    ...
    kay = await edit_or_reply(un_gmute, "`Sedang melakukan membuka Global Mute...`")
    ...
    await kay.edit("```Membuka Global Mute Pengguna...```")

    if ungmute(user.id) is False:
        await kay.edit("`Pengguna Sedang Tidak Di Gmute!`")
    else:
...
async def gspider(gspdr):
    ...
    kay = await edit_or_reply(gspdr, "`Processing...`")
    user, reason = await get_user_from_event(gspdr)
    if not user:
        return

    # If pass, inform and start gmuting
    await kay.edit("`Pengguna Berhasil Dibisukan!`")
    if gmute(user.id) is False:
        await edit_delete(gspdr, "`Kesalahan! Pengguna Sudah Dibisukan.`")
    else:
        if reason:
            await kay.edit(f"**Dibisukan Secara Global!**\n**Alasan:** `{reason}`")
        else:
